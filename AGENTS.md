...
- PIPELINE SYSTEM
  ...
  7.13 If verification stage is completed successfully, stage and commit the files, then open a pull request with the commits related to the issue and add a reference to the issue in the PR (eg. `Fixes #123`), so that the issue is also correlated with the PR and the issue is closed automatically on PR merge - this is because of the keyword `Fixes` (thus, do not attempt to close the issue again, as it will be already closed).
