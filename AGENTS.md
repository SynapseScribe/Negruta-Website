...
- PIPELINE SYSTEM
  ...
  7.16 When asked to `abort issue <#>. reason: <xyz>` -> move issue folder from active state to done (`.pipeline/done/<#>/`) -> close issue on gh, and any existing related pull request (Do not merge the pr in this case) -> add a comment with the `reason <xyz>`. 
